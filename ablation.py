...
def _inside_outside_with_spans(valid_spans, L, log_g, V):
    ...
    if not valid_spans: return None, -np.inf
...
class TopWordsTopicModel:
        ...
        while i < L:
            ...
            for l in range(min(self.max_w_len, L - i), 0, -1):
                ...
                if w in self.word_map:
                    ...
                    break
            if not found:
                # 如果没找到，跳过一个字符（虽然在当前模拟中通常都能找到单字）
                i += 1
        return res

    def _inside_outside(self, text, phi_t, phi_0, pi_dn):
        """传统 Inside-Outside (用于 Gibbs 采样似然计算)"""
        L = len(text)
        ...
        for i in range(L, 0, -1):
            for l in range(1, min(self.max_w_len, i) + 1):
                w = text[i-l:i]
                if w in self.word_map:
    ...
    def train(self, corpus, iterations=5, num_workers=4, tol=0.01):
        D = len(corpus)
        ...
        for it in range(iterations):
            ...
            doc_tasks = [
                (corpus[d], pi[d], self.theta[d], self.phi, self.word_map, self.max_w_len, self.K, self.V)
                for d in range(D)
            ]
            ...
            for t in range(self.K + 1):
                self.phi[t] = (new_n_ti[t] + self.beta_p) / (np.sum(new_n_ti[t]) + self.V * self.beta_p + 1e-20)
            for d in range(D):
                self.theta[d] = (new_n_dt[d] + self.alpha_p) / (np.sum(new_n_dt[d]) + self.K * self.alpha_p + 1e-20)
            ...
            logger.info(f"Iteration {it} | Likelihood: {total_likelihood:.2f} | F1: {metrics['f1']:.4f} | Topic Acc: {metrics['topic_acc']:.4f} | Time: {duration:.2f}s")
            
            # Early stopping check
            if it > 0:
                prev_likelihood = self.history['likelihood'][-2]
                improvement = (total_likelihood - prev_likelihood) / abs(prev_likelihood)
                if improvement < tol:
                    logger.info(f"Converged at iteration {it} (improvement {improvement:.4%} < {tol:.4%})")
                    break

    def train_lda(self, corpus, iterations=10):
        """Baseline 1: 仅 TopWords 分词 + 传统 LDA EM"""
        D = len(corpus)
        # 固定分词
        tokenized_corpus = []
        for d in range(D):
            doc_tokens = []
            for sent in corpus[d]['sentences']:
                doc_tokens.append(self.greedy_segment(sent['text']))
        ...
        for it in range(iterations):
            new_n_ti = np.zeros_like(self.phi[1:]) # 只更新主题 phi，不更新 phi[0]
            new_n_dt = np.zeros((D, self.K))
            total_likelihood = 0
            
            for d in range(D):
                for n, tokens in enumerate(tokenized_corpus[d]):
                    # 计算每个词在每个主题下的概率
                    log_phi_all = np.log(self.phi[1:] + 1e-20) # (K, V)
                    word_indices = [self.word_map[w] for w in tokens]
                    
                    # 简化 LDA 为句级别主题分配
                    log_G = np.sum(log_phi_all[:, word_indices], axis=1) # (K,)
            ...
            # M-Step
            for t in range(self.K):
                self.phi[t+1] = (new_n_ti[t] + self.beta_p) / (np.sum(new_n_ti[t]) + self.V * self.beta_p + 1e-20)
            for d in range(D):
                self.theta[d] = (new_n_dt[d] + self.alpha_p) / (np.sum(new_n_dt[d]) + self.K * self.alpha_p + 1e-20)

            # 评估
            dummy_pi = [[1.0 for _ in d['sentences']] for d in corpus]
            metrics = self.evaluate(corpus, dummy_pi)
            metrics['likelihood'] = total_likelihood
            for k, v in metrics.items(): self.history[k].append(v)
            logger.info(f"LDA Iter {it} | Likelihood: {total_likelihood:.2f} | F1: {metrics['f1']:.4f} | Topic Acc: {metrics['topic_acc']:.4f}")
    ...
    def train_gibbs(self, corpus, iterations=50):
        ...
        for d in range(D):
            for n in range(len(corpus[d]['sentences'])):
                n_dk[d, z[d][n]-1] += 1
        ...
        for it in range(iterations):
            for d in range(D):
                for n, sent in enumerate(corpus[d]['sentences']):
                    ...
                    # --- 计算采样分布 ---
                    # a. 先验项 (基于 alpha)
                    p_z = (n_dk[d] + self.alpha_p) / (np.sum(n_dk[d]) + self.K * self.alpha_p)
                    
                    # b. 似然项 (使用 Inside 算法计算总概率 G_k)
                    log_G = np.zeros(self.K)
                    for t in range(1, self.K + 1):
                        _, l_G = self._inside_outside(sent['text'], self.phi[t], self.phi[0], 0.5) # 简化 pi=0.5
                        log_G[t-1] = l_G
                    
                    # c. 组合并采样
                    max_log_G = np.max(log_G)
                    if max_log_G == -np.inf:
                        # 如果所有主题似然都是 -inf，则仅使用主题先验
                        probs = p_z
                    else:
                        probs = p_z * np.exp(log_G - max_log_G)
            ...
            metrics = self.evaluate(corpus, dummy_pi)
            for k, v in metrics.items(): self.history[k].append(v)
            logger.info(f"Gibbs Iter {it} | F1: {metrics['f1']:.4f} | Topic Acc: {metrics['topic_acc']:.4f}")

    def _update_parameters_after_sampling(self, corpus, z):
        D = len(corpus)
        new_n_ti = np.zeros_like(self.phi)
        new_n_dt = np.zeros((D, self.K))
        ...
        for d in range(D):
            for n, sent in enumerate(corpus[d]['sentences']):
                k_sampled = z[d][n]
                new_n_dt[d, k_sampled-1] += 1
                
                # 在采样到对应主题的情况下，计算 Inside-Outside 期望计数 (pi=0.5 简化)
                cnt, _ = self._inside_outside(sent['text'], self.phi[k_sampled], self.phi[0], 0.5)
                if cnt is not None:
                    # 分配给 Topic 和 Background
                    denom = 0.5 * self.phi[k_sampled] + 0.5 * self.phi[0] + 1e-20
                    q_s = (0.5 * self.phi[k_sampled]) / denom
                    new_n_ti[k_sampled] += q_s * cnt
                    new_n_ti[0] += (1 - q_s) * cnt
    ...
    def plot_metrics(self, save_path='training_metrics.png', params=None):
        ...
        ax1 = fig.add_subplot(1, 3, 1); ax1.plot(self.history['likelihood'], marker='o', color='steelblue'); ax1.set_title('Log Likelihood')
        ax2 = fig.add_subplot(1, 3, 2); ax2.plot(self.history['f1'], marker='o', color='forestgreen'); ax2.set_title('Segmentation F1')
        ax3 = fig.add_subplot(1, 3, 3); ax3.plot(self.history['topic_acc'], marker='o', color='darkorange'); ax3.set_title('Topic Accuracy')
        
        if params:
            param_str = "\n".join([f"{k}: {v}" for k, v in params.items() if k != 'num_workers'])
            fig.text(0.98, 0.5, param_str, va="center", ha="right", fontsize=11, 
...
class ModelDiagnostics:
    ...
    def analyze_all(model, corpus, pi_list, save_dir="results/diag"):
        ...
        for k in range(model.K + 1):
            name = "Background" if k == 0 else f"Topic {k} (Mapped to GT:{mapping.get(k-1, 'N/A')})"
...
# 3. 运行与评价
# ==========================================
def run_test(num_topics=2, alpha=0.1, beta=0.01, num_docs=1000, sents_per_doc=10, iterations=10, num_workers=4, plot=True, timestamp=None, tol=0.01):
    # 记录运行参数
    params = {
    ...
    sim = TopWordsTopicSimulator(num_topics=num_topics, alpha=alpha, beta=beta)
    ...
    model.train(corpus, iterations=iterations, num_workers=num_workers, tol=tol)
...
def run_distribution_test(n_runs=100, timestamp=None, **kwargs):
    ...
    ax1.hist(f1s, bins=min(20, n_runs), color='forestgreen', alpha=0.6, edgecolor='white')
    ax1.set_title(f'Segmentation F1 Distribution ({n_runs} runs)')
    ax1.set_xlabel('F1 Score')
    
    ax2 = fig.add_subplot(1, 2, 2)
    ax2.hist(accs, bins=min(20, n_runs), color='darkorange', alpha=0.6, edgecolor='white')
    ax2.set_title(f'Topic Accuracy Distribution ({n_runs} runs)')
    ax2.set_xlabel('Accuracy')
    
    params_for_display = {**kwargs}
    params_for_display['n_runs'] = n_runs
    param_str = "\n".join([f"{k}: {v}" for k, v in params_for_display.items() if k != 'num_workers'])
...
def run_comparison_test(target_param, param_list, n_runs=10, timestamp=None, **kwargs):
    ...
    fixed_params = {k: v for k, v in kwargs.items() if k != target_param and k != 'num_workers'}
...
def run_ablation_study(num_topics=5, alpha=0.1, beta=0.01, num_docs=1000, sents_per_doc=10, iterations=10, num_workers=4, tol=0.01):
    logger.info("\n" + "="*50 + "\nStarting Ablation Study (Comparison of 3 Versions)\n" + "="*50)
    
    # 1. 生成统一的测试语料
    sim = TopWordsTopicSimulator(num_topics=num_topics, alpha=alpha, beta=beta)
    corpus = sim.generate_corpus(num_docs=num_docs, sents_per_doc=sents_per_doc)
    logger.info(f"Generated corpus with {num_docs} docs.")

    results = {}

    # --- Baseline 1: Greedy Tokenization + Standard LDA EM ---
    logger.info("\n>>> [Baseline 1] Greedy Tokenization + Standard LDA EM")
    model_lda = TopWordsTopicModel(sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    model_lda.train_lda(corpus, iterations=iterations)
    results['Baseline 1 (Greedy+LDA)'] = {
        'f1': model_lda.history['f1'][-1],
        'topic_acc': model_lda.history['topic_acc'][-1]
    }

    # --- Baseline 2: Gibbs Sampling Version ---
    logger.info("\n>>> [Baseline 2] Gibbs Sampling (Inside-Outside based)")
    model_gibbs = TopWordsTopicModel(sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    # Gibbs 通常需要更多迭代，这里为了对比取相同/稍多
    model_gibbs.train_gibbs(corpus, iterations=min(iterations * 4, 100)) 
    results['Baseline 2 (Gibbs)'] = {
        'f1': model_gibbs.history['f1'][-1],
        'topic_acc': model_gibbs.history['topic_acc'][-1]
    }

    # --- Our Model: Parallel EM Version ---
    logger.info("\n>>> [Our Model] Parallel EM (Inside-Outside Hidden)")
    model_em = TopWordsTopicModel(sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    model_em.train(corpus, iterations=iterations, num_workers=num_workers, tol=tol)
    results['Our Model (Parallel EM)'] = {
        'f1': model_em.history['f1'][-1],
        'topic_acc': model_em.history['topic_acc'][-1]
    }

    # --- Print Comparison Table ---
    console = Console()
    table = Table(title="Ablation Study Results Comparison")
    table.add_column("Version", style="cyan")
    table.add_column("Segmentation F1", style="green")
    table.add_column("Topic Accuracy", style="magenta")

    for version, metrics in results.items():
        table.add_row(version, f"{metrics['f1']:.4f}", f"{metrics['topic_acc']:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="single", choices=["single", "dist", "compare", "ablation"])
    parser.add_argument("--runs", type=int, default=30, help="Number of runs for distribution/compare mode")
    ...
    parser.add_argument("--sents_per_doc", type=int, default=10, help="Sentences per document")
    parser.add_argument("--iterations", type=int, default=50, help="EM iterations")
    parser.add_argument("--tol", type=float, default=0.001, help="Convergence tolerance (fractional improvement in likelihood)")
    
    parser.add_argument("--target_param", type=str, default="num_topics", help="Parameter to vary in compare mode")
    parser.add_argument("--param_list", type=str, default="2,3,5,10", help="Comma-separated list of values for target_param")
    ...
    if args.mode == "single":
        ...
        run_distribution_test(n_runs=args.runs, timestamp=unified_timestamp, **sim_params)
        ...
        run_comparison_test(target_param=args.target_param, param_list=p_list, n_runs=args.runs, timestamp=unified_timestamp, **sim_params)
    elif args.mode == "ablation":
        run_ablation_study(**sim_params)
    else:
