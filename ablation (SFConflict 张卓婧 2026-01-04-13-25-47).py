...
class TopWordsTopicSimulator:
    ...
    def generate_corpus(self, num_docs=10, sents_per_doc=10):
        ...
        for d_id in range(num_docs):
            ...
            doc = {"doc_id": d_id, "theta_true": theta_d.tolist(),
                   "sentences": []}
            for n_id in range(sents_per_doc):
                ...
                for _ in range(random.randint(4, 8)):
                    ...
                    w_idx = np.random.choice(
                        len(self.word_dict), p=self.phi[source])
...
class TopWordsTopicModel:
    ...
    def _inside_outside(self, text, phi_t, phi_0, pi_dn):
        ...
        for i in range(L):
            for l in range(1, min(self.max_w_len, L - i) + 1):
                w = text[i:i+l]
                if w in self.word_map:
                    f[i+l] = np.logaddexp(f[i+l], f[i] +
                                          log_g[self.word_map[w]])
    ...
    def train(self, corpus, iterations=5, num_workers=4, tol=0.01):
        ...
        for it in range(iterations):
            ...
            doc_tasks = [
                (corpus[d], pi[d], self.theta[d], self.phi,
                 self.word_map, self.max_w_len, self.K, self.V)
                for d in range(D)
            ]
            ...
            for d in range(D):
                for n in range(len(pi[d])):
                    ...
                    denom = (sum(self.gamma_p) - 2) + \
                        c_spec_total[d][n] + c_bg_total[d][n]
            ...
            if it > 0:
                ...
                improvement = (total_likelihood -
                               prev_likelihood) / abs(prev_likelihood)
                if improvement < tol:
                    logger.info(
        ...
        for it in range(iterations):
            ...
            for d in range(D):
                for n, tokens in enumerate(tokenized_corpus[d]):
                    ...
                    # 简化 LDA 为句级别主题分配
                    log_G = np.sum(
                        log_phi_all[:, word_indices], axis=1)  # (K,)
            ...
            for k, v in metrics.items():
                self.history[k].append(v)
            logger.info(
                f"LDA Iter {it} | Likelihood: {total_likelihood:.2f} | F1: {metrics['f1']:.4f} | Topic Acc: {metrics['topic_acc']:.4f}")

    def train_gibbs(self, corpus, iterations=10):
        D = len(corpus)
        self.theta = np.random.dirichlet([self.alpha_p] * self.K, size=D)
        self.history = {'likelihood': [], 'f1': [], 'topic_acc': []}
        # 1. 随机初始化每个句子的主题 z_dn
        z = [[random.randint(1, self.K) for _ in d['sentences']]
             for d in corpus]
        ...
        for it in range(iterations):
            ...
            for k, v in metrics.items():
                self.history[k].append(v)
            logger.info(
                f"Gibbs Iter {it} | F1: {metrics['f1']:.4f} | Topic Acc: {metrics['topic_acc']:.4f}")
    ...
    def _update_parameters_after_sampling(self, corpus, z):
        ...
        for d in range(D):
            for n, sent in enumerate(corpus[d]['sentences']):
                ...
                cnt, _ = self._inside_outside(
                    sent['text'], self.phi[k_sampled], self.phi[0], 0.5)
                if cnt is not None:
                    # 分配给 Topic 和 Background
                    denom = 0.5 * self.phi[k_sampled] + \
                        0.5 * self.phi[0] + 1e-20
...
def run_test(num_topics=2, alpha=0.1, beta=0.01, num_docs=1000, sents_per_doc=10, iterations=10, num_workers=4, plot=True, timestamp=None, tol=0.01):
    ...
    corpus = sim.generate_corpus(
        num_docs=num_docs, sents_per_doc=sents_per_doc)
    model = TopWordsTopicModel(sim.word_dict, K=num_topics)
    model.train(corpus, iterations=iterations,
                num_workers=num_workers, tol=tol)
...
def run_distribution_test(n_runs=100, timestamp=None, **kwargs):
    ...
    ax1.hist(f1s, bins=min(20, n_runs), color='forestgreen',
             alpha=0.6, edgecolor='white')
    ax1.set_title(f'Segmentation F1 Distribution ({n_runs} runs)')
    ax1.set_xlabel('F1 Score')

    ax2 = fig.add_subplot(1, 2, 2)
    ax2.hist(accs, bins=min(20, n_runs), color='darkorange',
             alpha=0.6, edgecolor='white')
...
def run_comparison_test(target_param, param_list, n_runs=10, timestamp=None, **kwargs):
    ...
    fixed_params = {k: v for k, v in kwargs.items(
    ) if k != target_param and k != 'num_workers'}
    ...
    save_path = os.path.join(
        plot_dir, f"compare_{target_param}_{timestamp}.png")
    plt.savefig(save_path)
...
def run_ablation_study(num_topics=5, alpha=0.1, beta=0.01, num_docs=1000, sents_per_doc=10, iterations=10, num_workers=4, tol=0.01):
    ...
    logger.info("\n>>> [Baseline 1] Greedy Tokenization + Standard LDA EM")
    model_lda = TopWordsTopicModel(
        sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    ...
    model_gibbs = TopWordsTopicModel(
        sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    ...
    model_em = TopWordsTopicModel(
        sim.word_dict, K=num_topics, alpha=alpha, beta=beta)
    model_em.train(corpus, iterations=iterations,
                   num_workers=num_workers, tol=tol)
    ...
    for version, metrics in results.items():
        table.add_row(
            version, f"{metrics['f1']:.4f}", f"{metrics['topic_acc']:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="single",
                        choices=["single", "dist", "compare", "ablation"])
    ...
    parser.add_argument("--sents_per_doc", type=int,
                        default=10, help="Sentences per document")
    parser.add_argument("--iterations", type=int,
                        default=50, help="EM iterations")
    parser.add_argument("--tol", type=float, default=0.001,
                        help="Convergence tolerance (fractional improvement in likelihood)")

    parser.add_argument("--target_param", type=str, default="num_topics",
                        help="Parameter to vary in compare mode")
    parser.add_argument("--param_list", type=str, default="2,3,5,10",
    ...
    if args.mode == "single":
        ...
        run_comparison_test(target_param=args.target_param, param_list=p_list,
                            n_runs=args.runs, timestamp=unified_timestamp, **sim_params)
